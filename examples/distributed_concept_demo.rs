...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   📍 One engine processes ALL {} customers sequentially", customers.len());
    ...
    println!("   📊 Simulated execution time: {:?} (parallel)", distributed_duration);
    ...
    println!("├─────────────────┼──────────────┼───────────────┤");
    println!("│ Single Node     │ {:>10.1?} │ {:>11}   │", single_duration, single_total);
    println!("│ Distributed     │ {:>10.1?} │ {:>11}   │", distributed_duration, distributed_total);
    println!("└─────────────────┴──────────────┴───────────────┘");
...
fn create_test_customers() -> std::result::Result<Vec<Facts>, Box<dyn std::error::Error>> {
    ...
    for (name, age, tier, spent) in customers_data {
        ...
        let customer = FactHelper::create_user(name, age as i64, "email@test.com", "US", tier == "vip");
